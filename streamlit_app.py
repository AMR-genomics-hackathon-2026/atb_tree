...
TAXONIUM_URL = (
    ...
    "https://github.com/AMR-genomics-hackathon-2026/atb_tree"
    "/releases/latest/download/taxonium_metadata.tsv.gz"
)
TAXONIUM_BINARY_URL = (
    "https://taxonium.org/atb?addMetadataUrl="
    "https://github.com/AMR-genomics-hackathon-2026/atb_tree"
    "/releases/latest/download/taxonium_metadata_binary.tsv.gz"
)
...
@st.cache_data(show_spinner="Loading AMR metadata (~13 MB)…")
def load_data() -> pd.DataFrame:
    ...
    df["amr_present"] = df["amr_present"].map({"True": True, "False": False, True: True, False: False})
    return df
...
try:
    ...
    st.error(
        f"Could not load metadata from GitHub Release: {e}\n\n"
        "Make sure `taxonium_metadata.tsv.gz` has been uploaded to the latest release."
    )
    st.stop()
...
# Drug class filter (parsed from amr_class_summary)
all_classes = sorted(
    {c.strip() for row in df["amr_class_summary"].dropna() for c in row.split(";") if c.strip()}
)
...
n_species = filtered["Species"].nunique()
